...
reversals remain fragile. These results show that short-answer preference
...
directly rather than inferred from answer accuracy.
...
\begin{itemize}
    ...
    explicit probe but still struggles on relation reversals.
\end{itemize}
...
versus 88.2\% for \answerdpo{} after rounding. These external checks suggest
that the verifier rows do not cause a large out-of-distribution collapse, but
they mirror the internal trade-off: the trained models lower FNR while raising
FPR relative to Base Instruct.
...
improves explicit evidence consistency. The remaining relation failures are
the point of the benchmark: short-answer gains alone can hide persistent
