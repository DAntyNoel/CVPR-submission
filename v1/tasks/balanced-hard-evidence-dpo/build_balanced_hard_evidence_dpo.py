#!/usr/bin/env python3
"""Build Balanced Hard Evidence-DPO data from existing project artifacts."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        default=str(TASK_DIR / "generated/balanced_hard_evidence_dpo_train.jsonl"),
    )
    parser.add_argument(
        "--summary-output",
        default=str(TASK_DIR / "generated/balanced_hard_evidence_dpo_summary.json"),
    )
    parser.add_argument(
        "--llamafactory-dir",
        default=str(TASK_DIR / "llamafactory_data"),
    )
    parser.add_argument("--dataset-key", default="cvpr_balanced_hard_evidence_dpo")
    ...
    write_json(args.summary_output, build_summary(args, records))
    ...
    for row in rows:
        ...
        yes_row = {
            **common,
            ...
            "question": existence_question(pos),
            "target": "yes",
            "target_object": pos,
            "source": "balanced_base_error_pair",
            "task_type": "object_existence",
        }
        no_row = {
            **common,
            "id": f"{row['id']}::absent::{slug(neg)}",
            "question": existence_question(neg),
            "target": "no",
            "target_object": neg,
            "source": "balanced_base_error_pair",
            "task_type": "object_existence",
...
def select_canonical_coco_records(rows: list[dict[str, Any]], requested_rows: int, rng: random.Random) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        yes_row = {
            **common,
            "id": f"{row['id']}::present::{slug(pos)}",
            "question": row.get("question") or existence_question(pos),
            "target": "yes",
            "target_object": pos,
            "source": "balanced_canonical_coco_pair",
            "task_type": "object_existence",
        }
        no_row = {
            **common,
...
def select_gqa_anchor_records(rows: list[dict[str, Any]], requested_rows: int, rng: random.Random) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        anchors.append(
            {
                "id": f"{row['id']}::gqa_anchor",
                "image": row["image"],
                "image_id": row.get("image_id"),
                "prompt": prompt(row["question"]),
                "chosen": f"{row['chosen_answer']}\n{evidence_line(row['evidence_hint_chosen'])}",
                "rejected": f"{row['rejected_answer']}\n{evidence_line(row['evidence_hint_rejected'])}",
                "source": "balanced_gqa_anchor",
...
def paired_eval_rows(rows: list[dict[str, Any]], source_family: str) -> list[tuple[dict[str, Any], dict[str, Any]]]:
    ...
    for group_rows in grouped.values():
        ...
        yes_row["source"] = f"balanced_{source_family}_pair"
        no_row["source"] = f"balanced_{source_family}_pair"
        pairs.append((yes_row, no_row))
    return pairs
...
def object_dpo_record(row: dict[str, Any]) -> dict[str, Any]:
    ...
    if target == "yes":
        chosen = f"{yes_answer(target_object)}\n{support_evidence(target_object)}"
        rejected = f"{no_answer(target_object)}\n{unsupported_evidence(target_object)}"
    else:
        chosen = f"{no_answer(target_object)}\n{unsupported_evidence(target_object)}"
        rejected = f"{yes_answer(target_object)}\n{support_evidence(target_object)}"

    return {
        "id": row["id"],
        "image": row["image"],
        "prompt": prompt(existence_question(target_object)),
        "chosen": chosen,
        "rejected": rejected,
...
def evidence_line(hint: str) -> str:
    ...
    return f"Evidence: {hint.split(prefix, 1)[1].strip()}"


def support_evidence(obj: str) -> str:
    return f"Evidence: {article_phrase(obj)} {be_verb(obj)} visually supported in the image."


def unsupported_evidence(obj: str) -> str:
    return f"Evidence: {article_phrase(obj)} {be_verb(obj)} not visually supported in the image."


def yes_answer(obj: str) -> str:
    phrase = article_phrase(obj)
    return f"Yes, there {be_verb(obj)} {phrase} in the image."
