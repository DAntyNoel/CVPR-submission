...
using the selected three-group comparison: Base Instruct, CEPO Answer-DPO, and
CEPO-Dual.

Current files:

- `main.tex`: paper body with final CEPO-Probe benchmark framing, ClaimEvidence-6K
  data description, short-answer transfer results, evidence-probe results,
...
The CEPO-Probe PDF builds were verified on 2026-05-28: `build/main.pdf` is 4
pages and `build/main_full.pdf` is 6 pages. The remaining non-fatal warning is
the upstream `lineno.sty` UTF-8 warning from the bundled review style; there
are no current overfull table warnings or missing-reference warnings.
